...
void runSelfTest(Board b) {
  ...
  const size_t simpleCnt = genSimpleMovesNoPromote(b, movesSeparate);
  const size_t promoteCnt = genSimplePromotes(b, movesSeparate + simpleCnt);
  const size_t captureCnt = genCaptures(b, movesSeparate + simpleCnt + promoteCnt);
  if (captureCnt > BUFSZ_CAPTURES) {
    panic("Number of captures exceed BUFSZ_CAPTURES");
  }
  if (promoteCnt > BUFSZ_SIMPLE_PROMOTES) {
    panic("Number of simple promotes exceed BUFSZ_SIMPLE_PROMOTES");
  }
  ...
  const size_t allSimpleCnt = genSimpleMoves(b, movesAllSimple);
  ...
  std::sort(movesAllSimple, movesSeparate + allSimpleCnt, cmpMoves);
